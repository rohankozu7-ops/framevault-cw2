...
router.post("/", authMiddleware, upload.single("image"), async (req, res) => {
  try {
    ...
    const container = await getContainer("media");
    const newItem = {
      id: uuidv4(),
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    const { resources } = await container.items
      .query({ query: "SELECT * FROM c WHERE c.userId = @userId ORDER BY c.createdAt DESC", parameters: [{ name: "@userId", value: req.user.userId }] })
      .fetchAll();
    res.json(resources);
  } catch (err) {
    console.error(err);
...
router.delete("/:id", authMiddleware, async (req, res) => {
  try {
    const container = await getContainer("media");
    const { resources } = await container.items
      .query({ query: "SELECT * FROM c WHERE c.id = @id AND c.userId = @userId", parameters: [{ name: "@id", value: req.params.id }, { name: "@userId", value: req.user.userId }] })
      .fetchAll();
